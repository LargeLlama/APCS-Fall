...
public class Matrix 
{
    ...
    {	
		Object[] meme = new Object[this.size()];

		for (int i = 1; i < _matrix[r].length; i++) {
			meme[i] = this.get(r,i);
		}
		return meme;
	...
	//first check for proper dimension:	
		...
		System.out.println(newCol[2]);

		for( int row = 1; row < size()+1; row++ ) {
			foo[row-1] = get(row,c);
			set( row, c, newCol[row-1] );
			System.out.println("getting " + (row-1) + " element for newCol:" + newCol[row-1]);
    ...
    {
		Object[][]meme = new Object[this.size()][this.size()];
		for(int i = 0; i < this.size(); i++) {
			for(int n = 0; n < this.size(); n++) {
				meme[i][n] = this.get(n+1, i+1);
			}
		}
		_matrix = meme;
    }//O(n^2)
    ...
    public static void populate(Matrix meme) {
	  for (int i = 1; i < meme.size()+1; i ++) {
		  for (int n = 1; n < meme.size()+1; n++) {
	...
	//=========TESTING PHASE 2 JUHURDDDDDDDDDDDDDDDD===========
	Matrix meme = new Matrix(4);
	populate(meme);
	System.out.println(meme);
	
	Object[] test = {4, 3, 2, 1};
	
	System.out.println("testing set row\nrow 2 will be set to: {4, 3, 2, 1} ") ;
	meme.setRow(2,test);
	System.out.println(meme);

	System.out.println("testing set column\n column 3 will be set to: {4, 3, 2, 1}");
	meme.setCol(3,test);
	System.out.println(meme);
	System.out.println("\ntesting transpose");
	meme.transpose();
